...
// Time Limit: 1000
// Start: Thu Jul 24 12:33:07 2025
// mintemplate
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        n2 *= k + 1;
        ans1 *= k+1;
        ans2 *= (binpow(mint(x), k+1) - 1) * mint(x - 1).inv();
        D2 = D2 * (k + 1) % (2 * (mod - 1));
        sqrtN *= binpow(mint(x), k/2);
    }
    mint ans3;
    if(D2%2==0) {
        ll e = (D2/2) % (mod-1);
        ...
        ans3 = binpow(sqrtN, (ll)n2.v);
    }
    

    cout << ans1.v << " " << ans2.v << " " << ans3.v << nl;
}
