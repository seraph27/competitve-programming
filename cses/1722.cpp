...
// Time Limit: 1000
// Start: Thu Jul 24 17:26:09 2025
// mintemplate
#ifdef MISAKA
...
pair<mint,mint> fib(ll n) {
    if (n == 0) return {0, 1};
    auto [a, b] = fib(n >> 1);
    // a = F_k, b = F_{k+1} with k = n/2
    mint c = a * (2*b - a);      // F_{2k}   = F_k*(2*F_{k+1} − F_k)
    mint d = a*a + b*b;          // F_{2k+1} = F_k^2 + F_{k+1}^2
    if (n & 1) return {d, c + d};
    else        return {c, d};
}
void shiina_mashiro() {
    ll n; cin >> n;
    cout << fib(n).first.v << nl;
}
