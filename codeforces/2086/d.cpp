...
// Contest: Educational Codeforces Round 177 (Rated for Div. 2)
// URL: https://codeforces.com/contest/2086/problem/D
// Time Limit: 2000
// Start: 2025/04/03 23:09:15
// mintemplate
#include <bits/stdc++.h>
#define ll long long
#define ar array
...
void shiina_mashiro() {
    ...
    vector<mint> dp(sum+1, 0), dp2(sum+1, 0);
    dp[0] = 1;
    for (int i = 0; i < 26; i++){
        fill(all(dp2), 0);
        for (int j = 0; j <= sum; j++){
            ...
            dp2[j] += dp[j];
            if(vi[i] == 0) continue;
            if(j + vi[i] <= sum) dp2[j + vi[i]] += dp[j];
        }
        swap(dp, dp2);
    }

  
    int odd = (sum+1)/2, even = sum-odd;
    if(dp[odd].v == 0) {
        cout << 0 << nl;
        return;
    }
