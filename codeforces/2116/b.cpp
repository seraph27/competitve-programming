...
// Time Limit: 1000
// Start: Sat May 31 08:33:29 2025
// mintemplate
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    int idxP, idxQ;
    for(int i = 0; i < n; i++) {
        if(ckmax(mxP, p[i])) idxP = i;
        if(ckmax(mxQ, q[i])) idxQ = i;
        
        if(mxP > mxQ) {
            cout << binpow((mint)2, mxP) + binpow((mint)2, q[i-idxP]) << " ";
        } else if(mxP < mxQ) {
            cout << binpow((mint)2, mxQ) + binpow((mint)2, p[i-idxQ]) << " ";
        } else {
            mint good = (q[i-idxP] > p[i-idxQ] ? binpow((mint)2, q[i-idxP]) : binpow((mint)2, p[i-idxQ]));
            cout << good + binpow((mint)2, mxP) << " ";
        }
    }
    cout << nl;
}
