// Problem: C. k-Tree
// Contest: Codeforces Round 247 (Div. 2)
// URL: https://codeforces.com/contest/431/problem/C
// Time Limit: 1000
// Start: Thu Mar 26 11:25:56 2026
// multitest
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    for(int i = 0; i < 100; i++) {
        ...
        ndp[0][0] = 1;
        for(int curr = 0; curr < 101; curr++) for(int j = 1; j <= k; j++) for(int o = 0; o < 2; o++) {
            if(curr + j < 101) ndp[curr + j][(o || j>=d)] += dp[curr][o];
        }
        swap(dp, ndp);
    }
