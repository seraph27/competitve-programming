...
void shiina_mashiro() {
    ...
    vector<vector<mint>> dp(n+1, vector<mint>(n+1, 0)); //:= dp[i][j] = len, lst
    dp[0][0] = 1;
    for(int i = 0; i < n; i++) {
        for(int len = 1; len+i <= n; len++) { // add 
            vector<int> add(len);
            iota(all(add), 1);
            for(int s = 0; s < len; s++) {
                rotate(add.begin(), add.begin()+1, add.end());
                bool ok = 1;
                for(int L = i; L < i+len; L++) if(ban[L][add[L-i]-1]) {
                    ok = 0;
                    break;
                }
                if(!ok) continue;
                if(add[0]==1) { 
                    dp[i+len][len]+=reduce(all(dp[i]));
                } else{
                    mint sum = 0;
                    for(int k = 0; k <= n; k++) if(add[0] != k+1) sum+=dp[i][k];
                    dp[i+len][0] += sum;
                }
            }
        }
