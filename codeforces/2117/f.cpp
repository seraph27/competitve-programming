...
void shiina_mashiro() {
    ...
    if(sz(leaf) == 1) {
        cout << binpow((mint)2, depth[leaf[0]]+1) << nl;
        return;
    }
    debug(leaf);
    
    int dep1 = depth[leaf[0]];
    int dep2 = depth[leaf[1]];
    mint left = 3* binpow((mint)2, abs(dep1 - dep2));
    if(dep1 == dep2) {
        left = 4;
    }
    mint up = binpow((mint)2, split);
    debug(left.v, up.v, split);
    cout << (left * up).v << nl;
    debug(depth);
