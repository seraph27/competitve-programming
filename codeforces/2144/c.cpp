...
// Time Limit: 2000
// Start: Mon Sep 15 23:08:30 2025
// mintemplate
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    mint swp = 1, noswap = 1;
    for(int i = 1; i < n; i++) {
        mint nswap = 0, nnoswap = 0;
        if(a[i - 1] <= a[i] && b[i - 1] <= b[i]) nnoswap += noswap, nswap += swp;
        if(a[i - 1] <= b[i] && b[i - 1] <= a[i]) nswap += noswap, nnoswap += swp;
        swap(swp, nswap);
        swap(noswap, nnoswap);
    }
    cout << (swp + noswap).v << nl;
}
