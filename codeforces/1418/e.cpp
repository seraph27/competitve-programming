...
void shiina_mashiro() {
    ...
    for(int i = 0; i < n; i++) pref[i+1] += pref[i] + str[i];
    for(;m--;) {
        int hp, def; cin >> hp >> def;
        int weak = lower_bound(all(str), def) - str.begin();
        int strong = n - weak;
        debug(weak, strong);
        if(strong < hp) {
            cout << 0 << nl;
        ...
        mint ans = (mint)(strong - hp) / strong * (pref[n] - pref[weak]); 
        ans += (mint)(strong - hp + 1) / (strong + 1) * pref[weak];
        cout << ans << nl;
    } 
}
