...
// Time Limit: 1500
// Start: Thu Mar 19 00:45:19 2026
// multitest
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    for(int i = 2; i < MX; i++) {
        if(i - 1 >= 0) (dp[i][0] += dp[i - 1][0]) %= mod;
        if(i - 1 >= 0) (dp[i][0] += dp[i - 1][1]) %= mod;
        if(i - k >= 0) (dp[i][1] += dp[i - k][0]) %= mod;
        if(i - k >= 0) (dp[i][1] += dp[i - k][1]) %= mod;
    }
    
    vector<mint> pref(MX + 1);
    for(int i = 0; i < MX; i++) {
        pref[i + 1] += pref[i] + dp[i][0] + dp[i][1];
    }

    for(int i = 0; i < t; i++) {
        int a, b; cin >> a >> b;
        cout << pref[b + 1] - pref[a] << nl;
    }
}
