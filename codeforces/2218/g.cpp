...
void shiina_mashiro() {
    ...
    //thought process: 
    //1. propogate from 0, if adjacent cells are larger, you can mark them, if the final array is not all marked, answer is 0.
    //2. think about lower upper bound? for example at t = 0, 3 people sat already, the a_i can be anywhere from [1, 3] 
    //3. 1920 = 2^7 * 3 * 5?
    //4. lb is based on the last iteration how many people already sat down, ub is based on total people sat down?. 
    

    vector<int> vis(n), zeros;
    for(int i = 0; i < n; i++) {
        if (vi[i] == 0) zeros.pb(i), vis[i] = 1;
    }

    for(auto &idx : zeros) {
        for(int i = idx - 1; i >= 0; i--) {
            if(vis[i] || vi[i] <= vi[i + 1]) break;
            vis[i] = 1;
        }
    ...
    for(int i = 0; i < n; i++) {
        if(vi[i]) {
            int L = (i - 1 >= 0 ? vi[i - 1] : 1e9);
            int R = (i + 1 < n ? vi[i + 1] : 1e9);
            int mn = min(L, R);
            if(mn == vi[i] - 1) {
                ans *= (pref[vi[i]]);
            } else if(mn < vi[i] - 1) {
                ans *= (pref[vi[i]] - pref[vi[i] - 1]);
            }
        }
    }
    cout << ans.v << nl;
