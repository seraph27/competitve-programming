...
// Time Limit: 2000
// Start: Mon Aug 18 11:02:22 2025
// mintemplate
#ifdef MISAKA
...
mint dp[64][64]; //dp[i][j] := how many x s.t. < 2 ^ i and has popcount j
mint sum[64][64];

void init() {
    dp[0][0] = 1;
    for(int i = 0; i < 62; i++) for(int j = i; j >= 0; j--){
        dp[i + 1][j] += dp[i][j];
        dp[i + 1][j + 1] += dp[i][j]; 
        sum[i + 1][j] += sum[i][j];
        sum[i + 1][j + 1] += sum[i][j] + dp[i][j] * mint(1LL << i);
    }
}
...
void shiina_mashiro() {
    ...
    for(int i = 62; i >= 0; i--) {
        if((n >> i) & 1) {
            int need = k - cnt;
            if(need >= 0 && need <= i) {
                ans += sum[i][need] + dp[i][need] * pref;
            }
            cnt++;
            pref += mint(1LL << i);
        }
    }
    if(__builtin_popcountll(n) == k) ans += n;
    cout << ans.v << nl;
}
