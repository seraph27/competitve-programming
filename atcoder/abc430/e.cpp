...
// Time Limit: 2000
// Start: Tue Dec 23 07:03:25 2025
// mintemplate
#ifdef MISAKA
...
struct Hash {
    ...
    Hash(string s) : sz{(int)s.size()} {
        pref.resize(sz + 1, 0);
        for(int i = 0; i < sz; i++) {
            pref[i + 1] = pref[i] + pow * (s[i] == '0' ? 1 : 2);
            pow *= base;
        }
    }

    int getHash(int l, int r) {
        return (pref[r] - pref[l]).v;
    }
};
...
void shiina_mashiro() {
    ...
    Hash h1(a), h2(b);
    for(int i = 0; i < len; i++) {
        if(h1.getHash(i, i + len) * binpow((mint)base, i).inv() == h2.getHash(0, len)) {
            cout << i << nl;
            return;
        }
    }
